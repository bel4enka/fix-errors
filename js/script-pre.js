...
const openDropDownMenu = (e) => {
  const dropdownMenuNews = e.target.nextElementSibling;
  const menuItemArrow = document.querySelector(".navigation__button");

  dropdownMenuNews.classList.toggle("dropdown-menu_click-open");
  menuItemArrow.classList.toggle("navigation__button_arrow_up");
}

document.querySelector('.header').querySelector('.navigation__button').addEventListener("click", openDropDownMenu)
document.querySelector('.menu').querySelector('.navigation__button').addEventListener("click", openDropDownMenu)
...
const changeIndicator = (currentPositionOfScroll) => {
  ...
  let scrolled = (currentPositionOfScroll / endPositionOfScroll) * 100;
